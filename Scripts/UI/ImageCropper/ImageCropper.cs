...
namespace Gamla.GUI.ImageCropper
{
	public class ImageCropper : MonoBehaviour
	{
		...
		public static ImageCropper Instance
		{
			...
			{
				if (m_instance == null)
					m_instance = Instantiate(Resources.Load<ImageCropper>("Prefabs/ImageCropper"));
...
#pragma warning disable 0649
		...
		[SerializeField] private float autoZoomOutThreshold = 0.65f;

		[SerializeField] private float autoZoomInFillAmount = 0.64f;

		[SerializeField] private float autoZoomOutFillAmount = 0.51f;

		[SerializeField] private AnimationCurve autoZoomCurve;

		[SerializeField] private float m_selectionSnapToEdgeThreshold = 5f;

		public float SelectionSnapToEdgeThreshold
		{
			get { return m_selectionSnapToEdgeThreshold; }
		}

		[SerializeField] private float viewportScrollSpeed = 512f;

		[Header("Internal Variables")] [SerializeField]
		...
		[SerializeField] private RectTransform m_imageHolder;
		...
		[SerializeField] private RawImage m_orientedImage;

		public RawImage OrientedImage
		{
			get { return m_orientedImage; }
		}
		...
		[SerializeField] private RectTransform m_selectionGraphics;

		public RectTransform SelectionGraphics
		{
			get { return m_selectionGraphics; }
		}

		[SerializeField] private SizeChangeListener viewportSizeChangeListener;

		[SerializeField] private SelectionGraphicsSynchronizer selectionGraphicsSynchronizer;

		[SerializeField] private Behaviour[] ovalMaskElements;

		[SerializeField] private Behaviour[] guidelines;

		[Header("User Interface")] [SerializeField]
		private GameObject flipHorizontalButton;

		[SerializeField] private GameObject flipVerticalButton;

		[SerializeField] private GameObject rotate90DegreesButton;

		[SerializeField] private FontSizeSynchronizer textsSynchronizer;

		[Header("Crop Helpers")] [SerializeField]
		private Canvas cropRenderCanvas;

		[SerializeField] private RawImage cropRenderImage;

		[SerializeField] private RectTransform cropRenderSelection;

		[SerializeField] private Camera cropRenderCamera;
#pragma warning restore 0649
		...
		{
			get { return m_originalImageSize; }
		}

		private Vector2 m_orientedImageSize;

		public Vector2 OrientedImageSize
		{
			get { return m_orientedImageSize; }
		}

		public Vector2 SelectionSize
		{
			get { return m_selection.sizeDelta; }
		}
		...
		public void Show(Texture image, CropResult onCrop, Settings settings = null,
			ImageResizePolicy croppedImageResizePolicy = null)
		{
			if (image == null)
			{
			...
			minSize = minSize.ClampBetween(Vector2.one,
				Vector2.one * Mathf.Max(m_originalImageSize.x, m_originalImageSize.y));
			...
			rotate90DegreesButton.SetActive(
				(settings.visibleButtons & Button.Rotate90Degrees) == Button.Rotate90Degrees);

			Vector2 paddingMax = new Vector2(1f - Mathf.Clamp01(settings.selectionInitialPaddingRight),
				1f - Mathf.Clamp01(settings.selectionInitialPaddingTop));
			Vector2 paddingMin = new Vector2(Mathf.Clamp(settings.selectionInitialPaddingLeft, 0f, paddingMax.x),
				Mathf.Clamp(settings.selectionInitialPaddingBottom, 0f, paddingMax.y));
		...
		public void ResetView(bool frameSelection)
		{
			...
			minImageScale = Mathf.Min(m_viewportSize.x / m_orientedImageSize.x,
				m_viewportSize.y / m_orientedImageSize.y);
		...
		public Texture2D CropSelection(int width, int height)
		{
			...
			if (width > maxTextureSize || height > maxTextureSize)
			{
				...
				Debug.Log(string.Concat(width, "x", height, " is too large, using ", preferredWidth, "x",
					preferredHeight, " instead..."));
			...
			cropRenderCameraTR.position = cropRenderCanvasTR.TransformPoint(new Vector3(
				selectionCenter.x - m_orientedImageSize.x * 0.5f, selectionCenter.y - m_orientedImageSize.y * 0.5f,
				-5f));
		...
		public void SetOrientation(Orientation orientation)
		{
			...
			{
				selectionPosition = new Vector2(selectionPosition.y,
					m_orientedImageSize.x - selectionPosition.x - selectionSize.x);
				selectionSize = new Vector2(selectionSize.y, selectionSize.x);
			}
			else if (currentOrientation == Orientation.Rotate180)
				selectionPosition = new Vector2(m_orientedImageSize.x - selectionPosition.x - selectionSize.x,
					m_orientedImageSize.y - selectionPosition.y - selectionSize.y);
			else if (currentOrientation == Orientation.Rotate270)
			{
				selectionPosition = new Vector2(m_orientedImageSize.y - selectionPosition.y - selectionSize.y,
					selectionPosition.x);
				selectionSize = new Vector2(selectionSize.y, selectionSize.x);
			}
			else if (currentOrientation == Orientation.FlipHorizontal)
			...
			else if (currentOrientation == Orientation.Transpose)
			{
				...
				selectionPosition = new Vector2(selectionPosition.x,
					m_orientedImageSize.y - selectionPosition.y - selectionSize.y);
			else
			{
			...
			else if (orientation == Orientation.Rotate90)
			{
				...
				selectionPosition = new Vector2(m_originalImageSize.y - selectionPosition.y - selectionSize.y,
					selectionPosition.x);
				selectionSize = new Vector2(selectionSize.y, selectionSize.x);
			}
			else if (orientation == Orientation.Rotate180)
			{
				...
				selectionPosition = new Vector2(m_originalImageSize.x - selectionPosition.x - selectionSize.x,
					m_originalImageSize.y - selectionPosition.y - selectionSize.y);
			}
			else if (orientation == Orientation.Rotate270)
			{
				...
				selectionPosition = new Vector2(selectionPosition.y,
					m_originalImageSize.x - selectionPosition.x - selectionSize.x);
				selectionSize = new Vector2(selectionSize.y, selectionSize.x);
			}
		...
		public void StartAutoZoom(bool instantZoom)
		{
			...
			float fillRate = Mathf.Max(selectionSizeScaled.x / m_viewportSize.x,
				selectionSizeScaled.y / m_viewportSize.y);
			...
			else
			{
				...
				if (selectionBottomLeft.x < -1E-4f || selectionBottomLeft.y < -1E-4f ||
				    selectionTopRight.x > m_viewportSize.x + 1E-4f || selectionTopRight.y > m_viewportSize.y + 1E-4f)
					zoomAmount = scale;
			}
		...
		private IEnumerator AutoZoom(float zoomAmount, bool instantZoom)
		{
			...
			Vector2 finalPosition = m_viewportSize * 0.5f -
			                        (m_selection.anchoredPosition + m_selection.sizeDelta * 0.5f) * finalScale.z;
		...
		{
			Vector2 size = m_selection.sizeDelta.RoundToInt()
				.ClampBetween(currMinSize.CeilToInt(), m_orientedImageSize);
			...
			m_selection.anchoredPosition = m_selection.anchoredPosition.RoundToInt()
				.ClampBetween(Vector2.zero, m_orientedImageSize - size);
		...
		public void UpdateSelection(Vector2 position)
		{
			m_selection.anchoredPosition =
				position.ClampBetween(Vector2.zero, m_orientedImageSize - m_selection.sizeDelta);
		}

		public void UpdateSelection(Vector2 position, Vector2 size, Direction pivot = Direction.None,
