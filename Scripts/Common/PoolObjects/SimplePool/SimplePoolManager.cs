...
namespace Gamla.Logic
{
	public class SimplePoolManager : MonoBehaviour
	{
		...
		public static PoolObject Spawn(string path, Vector3 position, Quaternion rotation, bool isAddressable,
			Transform parent)
		{
			return instance.SpawnObject(path, position, rotation, isAddressable, parent);
		}

		public PoolObject SpawnObject(string path, Vector3 position, Quaternion rotation, bool isAddressable,
			Transform parent)
		{
