...
namespace Gamla.Logic
{
	public sealed class ObjectPoolFactory<TObjectType> : IObjectFactory, IObjectDisposer<TObjectType>
	{
		...
		public TCreateObject CreateObject<TCreateObject>() where TCreateObject : class
		{
			...
			catch (Exception exception)
			{
				throw new CannotCreateResourceException(
					string.Format(@"Cannot create resource with path {0} and id: {1}", _resourcePath, _resourceId),
					exception);
			}
			...
			if (component == null)
			{
				throw new CannotCreateResourceException(string.Format(
					@"Component of type: {0} not exist on resources: {1}", typeof(TCreateObject).Name, _resourceId));
			}
		...
		public void DisposeItem(TObjectType objectRef)
		{
			...
			{
				throw new CannotDisposeItemException(
					string.Format(@"Object: {0} not created in Factory and cannot be destroyed", objectRef));
			}
